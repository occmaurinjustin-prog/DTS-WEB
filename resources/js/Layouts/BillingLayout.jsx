...
function Sidebar({ activeMenu, authUser }) {
    const menuItems = [
        { id: 'dashboard', name: 'Dashboard', icon: 'dashboard', href: '/billing/dashboard' },
        { id: 'invoices', name: 'Invoices', icon: 'invoices', href: '/billing/invoices' },
        { id: 'payments', name: 'Payments', icon: 'payments', href: '/billing/payments' },
        { id: 'clients', name: 'Clients', icon: 'clients', href: '/billing/clients' },
        { id: 'reports', name: 'Reports', icon: 'reports', href: '/billing/reports' },
        { id: 'payroll', name: 'Payroll', icon: 'payments', href: '/billing/payroll' },
        { id: 'settings', name: 'Settings', icon: 'settings', href: '/billing/settings' },
    ];
    ...
    return (
        <div className="w-[280px] h-screen bg-slate-950/95 backdrop-blur-2xl p-6 flex flex-col fixed left-0 top-0 z-50 rounded-r-[2.5rem] border-r border-white/5 shadow-[8px_0_32px_rgba(0,0,0,0.3)]">
            ...
            <div className="flex items-center gap-4 mb-12 px-2 group cursor-pointer">
                <div className="w-12 h-12 bg-gradient-to-br from-blue-400 to-cyan-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/30 group-hover:shadow-blue-500/50 transition-all duration-300 group-hover:scale-105 relative overflow-hidden">
                    <div className="absolute inset-0 bg-white/20 blur-md opacity-0 group-hover:opacity-100 transition-opacity" />
                    <Icon name="payments" className="w-6 h-6 text-white relative z-10" />
                </div>
                <div>
                    <h1 className="text-white font-black text-xl leading-tight tracking-tight group-hover:text-blue-400 transition-colors">DTS</h1>
                    <p className="text-blue-500/80 text-[10px] uppercase font-bold tracking-[0.2em]">Billing Portal</p>
                </div>
            </div>
                    ...
                    return (
                        <Link
                            ...
                            className={`flex items-center gap-3.5 px-4 py-3.5 rounded-2xl transition-all duration-300 group relative overflow-hidden ${
                                isActive 
                                    ? 'bg-gradient-to-r from-blue-500/15 to-blue-500/5 text-blue-400 border border-blue-500/20 shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)]' 
                        ...
                        >
                            ...
                            <Icon name={item.icon} className={`w-5 h-5 transition-transform duration-300 ${isActive ? 'text-blue-400 scale-110' : 'text-slate-500 group-hover:text-slate-300 group-hover:scale-110'}`} />
                ...
                <div className="bg-slate-900/50 border border-white/5 rounded-2xl p-3 flex flex-col gap-3 backdrop-blur-md transition-colors">
                    <div className="flex items-center gap-3">
                        {authUser?.profile_image ? (
                            ...
                            <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-cyan-600 rounded-xl flex items-center justify-center shadow-inner relative">
                                <div className="absolute inset-0 bg-white/20 rounded-xl" />
                                <span className="text-white font-black text-sm relative z-10">
                                    {(authUser?.username || 'B').charAt(0).toUpperCase()}
                                </span>
                            </div>
                        )}
                        <div className="flex-1 min-w-0">
                            <p className="text-slate-100 text-sm font-bold truncate tracking-wide">{authUser?.username || 'Billing'}</p>
                            <p className="text-blue-500/70 text-[10px] uppercase font-bold tracking-wider truncate">Billing Dept</p>
                        </div>
                        <LogoutButton />
                    </div>
...
export default function BillingLayout({ children, title, authUser, activeMenu }) {
    const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

    return (
        <>
            <Head title={title || 'Billing Dashboard'} />
            ...
            <div className="min-h-screen bg-slate-50 flex selection:bg-blue-500/30">
                ...
                <div className="fixed inset-0 pointer-events-none z-0">
                    ...
                    <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-blue-50/50 rounded-full blur-[100px] -translate-y-1/2 translate-x-1/3" />
                </div>
                ...
                <div className="flex-1 flex flex-col min-w-0 lg:ml-[280px] relative z-10">
                    ...
                    <header className="lg:hidden bg-white/80 backdrop-blur-xl border-b border-slate-200/50 px-5 py-4 flex items-center justify-between sticky top-0 z-30">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-gradient-to-br from-blue-400 to-cyan-600 rounded-xl flex items-center justify-center shadow-md shadow-blue-500/20">
                                <Icon name="payments" className="w-5 h-5 text-white" />
                            </div>
                            <div>
                                <span className="text-slate-900 font-black text-lg tracking-tight">DTS</span>
                                <p className="text-blue-600 text-[9px] uppercase font-bold tracking-widest leading-none">Billing</p>
                            </div>
                        </div>
                        <button 
                            onClick={() => setMobileMenuOpen(true)}
                            className="p-2.5 text-slate-500 hover:text-blue-600 bg-slate-100 hover:bg-blue-50 rounded-xl transition-colors"
                        >
